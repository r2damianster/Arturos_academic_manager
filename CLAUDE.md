...
- **Visualización de planes cumplidos (estado="cumplido")**: La opción de ver como solo lectura y copiar planes "cumplidos" (pase de lista realizado) desde la agenda está parcialmente implementada pero no funciona completamente. **Causa probable**: El `PlanificarModal` recibe la prop `readOnly=true` desde agenda-client.tsx cuando `bitacoraMap.estado='cumplido'`, pero los campos pueden no estar deshabilitados correctamente o la UI no muestra claramente que es solo lectura. **Estado**: El query en agenda-client.tsx (línea 269) trae todas las bitácoras sin filtro de estado, por lo que los datos están disponibles; el problema es en la presentación/interacción del componente.
- **Edición de asistencia en fechas pasadas**: Se desconoce si hay restricción explícita. **Causa probable**: `registrarAsistenciaMasiva()` en asistencia.ts usa `upsert`, lo que teóricamente permite editar asistencia pasada. Verificar si hay validación de fecha en PaseListaClient o restricción en RLS de tabla `asistencia`.
