-- ============================================================
-- Sistema de Grupos en Clases (idempotente — seguro re-ejecutar)
-- ============================================================
...
('Países',   ARRAY['Albania','Brasil','Chile','Dinamarca','Egipto','Francia','Grecia','Holanda','Italia','Japón','Kenya','Líbano','México','Nepal','Omán','Perú','Qatar','Rusia','Suecia','Túnez'], 1),
('Colores',  ARRAY['Rojo','Azul','Verde','Amarillo','Naranja','Morado','Rosa','Turquesa','Café','Blanco'], 2),
('Planetas', ARRAY['Mercurio','Venus','Tierra','Marte','Júpiter','Saturno','Urano','Neptuno'], 3),
('Animales', ARRAY['Águila','Búho','Cóndor','Delfín','Elefante','Flamenco','Guepardo','Halcón','Iguana','Jaguar'], 4),
...
DROP TRIGGER IF EXISTS trg_unique_grupo_sesion ON public.grupo_integrantes;
...
DROP POLICY IF EXISTS "todos_leen_categorias"         ON public.grupo_categorias;
DROP POLICY IF EXISTS "profesor_gestiona_grupos"       ON public.grupos_clase;
DROP POLICY IF EXISTS "estudiante_ve_grupos_afinidad"  ON public.grupos_clase;
DROP POLICY IF EXISTS "profesor_gestiona_integrantes"  ON public.grupo_integrantes;
DROP POLICY IF EXISTS "estudiante_ve_sus_integrantes"  ON public.grupo_integrantes;
DROP POLICY IF EXISTS "estudiante_se_une_grupo"        ON public.grupo_integrantes;
DROP POLICY IF EXISTS "estudiante_sale_grupo"          ON public.grupo_integrantes;
DROP POLICY IF EXISTS "profesor_gestiona_participacion" ON public.grupo_participacion;
...
CREATE POLICY "profesor_gestiona_integrantes"
  ...
  USING (
    EXISTS (SELECT 1 FROM public.grupos_clase g WHERE g.id = grupo_id AND g.profesor_id = auth.uid())
  );

CREATE POLICY "estudiante_ve_sus_integrantes"
  ON public.grupo_integrantes FOR SELECT
  USING (
    EXISTS (SELECT 1 FROM public.estudiantes e WHERE e.id = estudiante_id AND e.auth_user_id = auth.uid())
  );
...
CREATE POLICY "estudiante_se_une_grupo"
  ...
  WITH CHECK (
    EXISTS (SELECT 1 FROM public.estudiantes e WHERE e.id = estudiante_id AND e.auth_user_id = auth.uid())
    AND EXISTS (SELECT 1 FROM public.grupos_clase g WHERE g.id = grupo_id AND g.abierto = true AND g.tipo = 'afinidad')
  );
...
CREATE POLICY "estudiante_sale_grupo"
  ...
  USING (
    EXISTS (SELECT 1 FROM public.estudiantes e WHERE e.id = estudiante_id AND e.auth_user_id = auth.uid())
    AND EXISTS (SELECT 1 FROM public.grupos_clase g WHERE g.id = grupo_id AND g.abierto = true)
  );
